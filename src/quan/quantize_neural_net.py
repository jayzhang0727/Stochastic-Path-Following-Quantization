...
QUAN_ACT_SYMMETRIC = False # if we use symmetric quantization for activations
...
class QuantizeNeuralNet:
    ...
    '''
    def __init__(self, network_to_quantize, network_name, batch_size, data_loader, 
        ...
        act_bits: int
            Num of bits used for activation quantization.
        ...
        calib_loader: function,
            The generator that loads the calibration dataset
        ...
        if quan_act:
            ...
            self.calib_data_loader_iter = iter(calib_loader)
            
            if QUAN_ACT_SYMMETRIC:
                self.act_boundary_idx = 2 ** (act_bits - 1)
            else:
                self.act_boundary_idx = 2 ** act_bits 
    ...
    def quantize_network(self):
        ...
        if self.quan_act:
            print('Start to quantize the activations using calibration data...')
            calib_data = next(self.calib_data_loader_iter)[0].to(self.device)
            
            for layer_idx in tqdm(layers_to_quantize):
                act_hook = ActHook(self.act_boundary_idx)
                self.quantized_network_layers[layer_idx].register_forward_pre_hook(act_hook)

            with torch.no_grad():
                self.quantized_network(calib_data)  # calibrate the step size for activation quantization
        ...
        print('Start weight quantization...')
        counter = 0
        ...
        for layer_idx in layers_to_quantize:
            ...
            if type(self.analog_network_layers[layer_idx]) == LINEAR_MODULE_TYPE:
                ...
                Q, quantize_error, relative_quantize_error, quantize_adder, relative_adder = quantize_layer(
                                                W, 
                                                analog_layer_input, 
    ...
    def _populate_linear_layer_input(self, layer_idx):
        ...
        # get data
        raw_input_data = next(self.data_loader_iter)[0].to(self.device)
...
class ActHook:
    """
    This class is used to quantize activations
    """
    def __init__(self, act_boundary_idx):
        self.calib = True
        self.step_size = None
        self.boundary_idx = act_boundary_idx

    def __call__(self, module, module_in):
        '''
        Process the input to the attached layer and set step size
        '''
        if len(module_in) != 1:
        ...
        if self.calib:
            self.step_size = x.view(x.size(0), -1).abs().max(dim=1).values.mean() / self.boundary_idx
            self.calib = False
            return x
            
        return msq(self.step_size, x, self.boundary_idx)
