...
from quan import QuantizeNeuralNet
...
parser.add_argument('--bits_act', '-ba', default=[8], type=int, nargs='+',
...
def main(b, ba, mlp_s, cnn_s, bs, mlp_per, cnn_per, l):
    bits = b
    bits_act = ba
    batch_size = bs
    calib_bs = max(batch_size, args.calib_bs)
    ...
    if args.data_set == 'ILSVRC2012':
        model = getattr(torchvision.models, args.model)(weights='IMAGENET1K_V1')
    ...
    if args.store_quan: # store quantized model
        if args.quan_act:
...
if __name__ == '__main__':
    ...
    params = [(b, ba, s, s, bs, mlp_per, cnn_per, l) 
                            for b in args.bits
                            for ba in args.bits_act
    ...
    # testing section
    for b, ba, mlp_s, cnn_s, bs, mlp_per, cnn_per, l in params:
        main(b, ba, mlp_s, cnn_s, bs, mlp_per, cnn_per, l)
